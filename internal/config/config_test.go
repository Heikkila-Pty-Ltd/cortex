...
func TestLoadDispatchCostControlDefaults(t *testing.T) {
	path := writeTestConfig(t, validConfig)
	loaded, err := Load(path)
	if err != nil {
		t.Fatalf("Load failed: %v", err)
	...
	cc := loaded.Dispatch.CostControl
	if cc.RetryEscalationAttempt != 2 {
		t.Fatalf("retry escalation default = %d, want 2", cc.RetryEscalationAttempt)
	}
	if cc.ComplexityEscalationMinutes != 120 {
		t.Fatalf("complexity escalation default = %d, want 120", cc.ComplexityEscalationMinutes)
	}
	if cc.StageAttemptWindow.Duration != 6*time.Hour {
		t.Fatalf("stage attempt window default = %v, want 6h", cc.StageAttemptWindow.Duration)
	}
	if cc.StageCooldown.Duration != 45*time.Minute {
		t.Fatalf("stage cooldown default = %v, want 45m", cc.StageCooldown.Duration)
	}
	if len(cc.RiskyReviewLabels) == 0 {
		t.Fatal("expected default risky review labels to be set")
	}
}

func TestLoadDispatchCostControlCustomValues(t *testing.T) {
	cfg := validConfig + `

[dispatch.cost_control]
enabled = true
spark_first = true
retry_escalation_attempt = 3
complexity_escalation_minutes = 180
risky_review_labels = ["risk:high", "security"]
force_spark_at_weekly_usage_pct = 90
daily_cost_cap_usd = 25.5
per_bead_cost_cap_usd = 7.5
per_bead_stage_attempt_limit = 2
stage_attempt_window = "4h"
stage_cooldown = "30m"
`
	...
	cc := loaded.Dispatch.CostControl
	if !cc.Enabled {
		t.Fatal("expected dispatch.cost_control.enabled=true")
	}
	if !cc.SparkFirst {
		t.Fatal("expected dispatch.cost_control.spark_first=true")
	}
	if cc.RetryEscalationAttempt != 3 {
		t.Fatalf("retry escalation = %d, want 3", cc.RetryEscalationAttempt)
	}
	if cc.ComplexityEscalationMinutes != 180 {
		t.Fatalf("complexity escalation = %d, want 180", cc.ComplexityEscalationMinutes)
	}
	if cc.ForceSparkAtWeeklyUsagePct != 90 {
		t.Fatalf("force spark weekly pct = %v, want 90", cc.ForceSparkAtWeeklyUsagePct)
	}
	if cc.DailyCostCapUSD != 25.5 {
		t.Fatalf("daily cost cap = %v, want 25.5", cc.DailyCostCapUSD)
	}
	if cc.PerBeadCostCapUSD != 7.5 {
		t.Fatalf("per bead cost cap = %v, want 7.5", cc.PerBeadCostCapUSD)
	}
	if cc.PerBeadStageAttemptLimit != 2 {
		t.Fatalf("stage attempt limit = %d, want 2", cc.PerBeadStageAttemptLimit)
	}
	if cc.StageAttemptWindow.Duration != 4*time.Hour {
		t.Fatalf("stage attempt window = %v, want 4h", cc.StageAttemptWindow.Duration)
	}
	if cc.StageCooldown.Duration != 30*time.Minute {
		t.Fatalf("stage cooldown = %v, want 30m", cc.StageCooldown.Duration)
	}
}

func TestLoadDispatchCostControlInvalidThreshold(t *testing.T) {
	cfg := validConfig + `

[dispatch.cost_control]
enabled = true
force_spark_at_weekly_usage_pct = 101
`

	path := writeTestConfig(t, cfg)
	_, err := Load(path)
	if err == nil {
		t.Fatal("expected validation failure for force_spark_at_weekly_usage_pct")
	}
	if !strings.Contains(err.Error(), "force_spark_at_weekly_usage_pct") {
