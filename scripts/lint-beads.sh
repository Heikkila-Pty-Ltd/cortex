...
tmp_ready="$(mktemp)"
tmp_merged="$(mktemp)"
trap 'rm -f "$tmp_open" "$tmp_inprog" "$tmp_all" "$tmp_general" "$tmp_ready" "$tmp_merged"' EXIT
...
ready_violations="$(
  jq -r '
    ...
    def ac_text: (.acceptance_criteria | norm);
    def has_label($label):
      ((.labels // []) | map((tostring | ascii_downcase)) | index(($label | ascii_downcase))) != null;

    .[]
    | select(has_label("stage:ready"))
    | . as $issue
    | ([
        if (($issue.acceptance_criteria | blank)) then "missing_acceptance_criteria" else empty end,
        if ((ac_text | ascii_downcase | test("test|unit test|integration test|e2e"))) then empty else "missing_test_requirement_in_acceptance" end,
        if ((ac_text | ascii_downcase | test("dod|definition of done"))) then empty else "missing_dod_requirement_in_acceptance" end,
        if (((.estimated_minutes // 0) | tonumber? // 0) > 0) then empty else "missing_estimated_minutes" end,
        if (($issue.design | blank)) then "missing_design_notes" else empty end
      ]) as $errs
    | select(($errs | length) > 0)
    | [$issue.id, $issue.status, ($issue.issue_type // ""), ($issue.title // ""), ($errs | join(","))]
    | @tsv
...
[[ -n "${ready_violations}" ]] && printf '%s\n' "${ready_violations}" >"$tmp_ready"
cat "$tmp_general" "$tmp_ready" >"$tmp_merged"
...
if [[ -s "$tmp_merged" ]]; then
  ...
  if [[ -s "$tmp_general" ]]; then
    ...
    awk -F'\t' '{printf("  - %s [%s/%s]: %s (%s)\n", $1, $2, $3, $4, $5)}' "$tmp_general" >&2
  fi
  if [[ -s "$tmp_ready" ]]; then
    echo "lint-beads: stage:ready gate violations (requires acceptance+test+DoD, estimate>0, design):" >&2
    awk -F'\t' '{printf("  - %s [%s/%s]: %s (%s)\n", $1, $2, $3, $4, $5)}' "$tmp_ready" >&2
  fi
  exit 1
fi

count="$(jq 'length' "$tmp_all")"
ready_count="$(jq '[.[] | select(((.labels // []) | map(tostring) | index("stage:ready")) != null)] | length' "$tmp_all")"
echo "lint-beads: PASS (${count} open/in_progress beads validated, ${ready_count} stage:ready beads gated)"
